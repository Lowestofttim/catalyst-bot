...
class MarketDatabaseBoundaryTests(unittest.TestCase):
    def test_sage_single_offer_test_uses_database_helper_for_spare_coins(self):
        ...
        with (
            ...
            patch(
                "sqlite3.connect", side_effect=AssertionError("raw sqlite connect used")
            ),
            patch(
                "database.get_smallest_free_tier_spare",
                side_effect=fake_spare,
                create=True,
            ) as spare,
        ):
        ...
        self.assertEqual(
            [call.args[0] for call in spare.call_args_list], ["xch", "cat"]
        )
