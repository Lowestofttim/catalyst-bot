...
def main():
    ...
    t.log(
        f"  Mode: {'DRY RUN (no transactions)' if DRY_RUN else 'LIVE (real transactions)'}"
    )
    t.log(f"{'=' * 60}\n")
    ...
    try:
        ...
        if coins:
            ...
            t.pass_test(
                "Query coins", f"{len(coins)} coins, total {total / 1e12:.6f} XCH"
            )

            # Show what we have
            t.log("      Coins:")
            for i, c in enumerate(
                sorted(coins, key=lambda x: x["amount"], reverse=True)[:5]
            ):
                cid = c["coin_id"]
    ...
    try:
        ...
        if len(pending) == 0:
            ...
            t.fail_test(
                ...
                f"{len(pending)} pending — wait for them to confirm!",
            )
    ...
    try:
        ...
        if spendable:
            t.pass_test(
                "Biggest coin spendable",
                f"{biggest_id[:16]}... ({biggest['amount'] / 1e12:.4f} XCH)",
            )
        else:
            t.fail_test(
                "Biggest coin NOT spendable",
    ...
    if biggest["amount"] < TEST_XCH_MOJOS * 2:
        t.fail_test(
            "Insufficient balance", f"Need at least {TEST_XCH_MOJOS * 2 / 1e12:.4f} XCH"
        )
        t.summary()
        return
    ...
    if change_after and change_after["amount"] == change_coin["amount"]:
        t.pass_test(
            "Change coin survived!",
            f"Same size: {change_after['amount'] / 1e12:.6f} XCH",
        )
    elif change_after:
        t.fail_test(
            "Change coin DIFFERENT size!",
    ...
    t.log(
        f"   Total: {len(final_coins)} coins = {sum(c['amount'] for c in final_coins) / 1e12:.6f} XCH"
    )
