...
def sage_rpc(endpoint, payload, timeout=15):
    ...
    conn = http.client.HTTPSConnection(
        SAGE_HOST, SAGE_PORT, timeout=timeout, context=ctx
    )
    conn.request(
        "POST",
        "/" + endpoint.lstrip("/"),
...
if len(xch_coins) < NUM_TEST or len(cat_coins) < NUM_TEST:
    print(
        f"  [SKIP] Need at least {NUM_TEST} coins per side, have {len(xch_coins)} XCH + {len(cat_coins)} CAT"
    )
...
print()
print(
    f"--- Step 4: Sequential creation ({NUM_TEST} buy + {NUM_TEST} sell = {NUM_TEST * 2} offers) ---"
)
sequential_ids = []
...
print(
    f"  Sequential total: {sequential_total:.0f}ms ({sequential_total / 1000:.1f}s) -- {seq_created}/{NUM_TEST * 2} created"
)
...
print()
print(
    f"--- Step 5: Parallel creation ({NUM_TEST} buy + {NUM_TEST} sell, ThreadPoolExecutor) ---"
)
...
if len(xch_coins) < NUM_TEST or len(cat_coins) < NUM_TEST:
    ...
    if parallel_times:
        ...
        print(
            f"  Avg per offer: {avg_ms:.0f}ms, Min: {min(parallel_times):.0f}ms, Max: {max(parallel_times):.0f}ms"
        )
    ...
    print("=" * 70)
    print(
        f"  Sequential ({NUM_TEST * 2} offers): {sequential_total:.0f}ms ({sequential_total / 1000:.1f}s) -- {seq_created} created"
    )
    print(
        f"  Parallel   ({NUM_TEST * 2} offers): {parallel_total:.0f}ms ({parallel_total / 1000:.1f}s) -- {par_created} created"
    )
    ...
    if sequential_total > 0 and parallel_total > 0:
        ...
        if par_created == NUM_TEST * 2 and speedup > 2:
            print()
            print(
                f"  [OK] Parallel creation works! {speedup:.1f}x faster with {MAX_WORKERS} workers."
            )
            print(
                f"  Recommended: ThreadPoolExecutor(max_workers={MAX_WORKERS}) for production."
            )
            ...
            if fail_rate > 30:
                ...
                print(
                    "  Acceptable — some coin conflicts expected. Retry handles them."
                )
        else:
            print()
            print(
                "  [OK] Parallel works but minimal speedup. Sage serializes internally."
            )
